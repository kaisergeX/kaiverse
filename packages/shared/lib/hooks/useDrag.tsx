...
export const useDrag = <T extends HTMLElement = HTMLElement>(
  ...
  const isSupportTranslate = typeof window !== 'undefined' ? CSS.supports('translate', '0') : false
  ...
  const handlePointerMove = useCallback(
    (e: PointerEvent) => {
      ...
      if (opts.decelerationEffect) {
        ...
        const {limittedPosition} = getImposedLimitPosition({x, y}, opts.direction, opts.limit)
