...
<Aside type="caution">
  Some components like <a href={`${import.meta.env.BASE_URL}/components/dialog`}>`<Dialog>`</a> have associated compound components. Compound
...
Since `@kaiverse/k/ui` component CSS rules are applied within `k-components` layer. If you are facing issues with TailwindCSS styles overriding `@kaiverse/k/ui` styles or vice versa,
...
<Code
  ...
  ins={[1]}
  lang="css"
  code={`@layer base, k-components, theme, components, utilities;
