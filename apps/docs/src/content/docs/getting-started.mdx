...
<Aside>
  `@kaiverse/k/ui` is not a common React component library. It's a collection of rarely used
  components.
</Aside>
...
To use `@kaiverse/k` with NextJS, adding the following to your `next.config.mjs`:
...
Enable the experimental flag `experimentalReactChildren` to make it work via `astro.config.mjs`
file.
