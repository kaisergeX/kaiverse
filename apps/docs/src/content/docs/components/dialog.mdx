...
<Tabs>
  <TabItem label="Playground">
    <Demo path="dialog/controlled-by-form" client:load />
  </TabItem>
  <TabItem label="Source code">
...
For instance, here is a drawer with the same UI as the previous "Controlled by form" example.

<Tabs>
  <TabItem label="Playground">
    <Demo path="dialog/without-compound" client:load />
  </TabItem>
  <TabItem label="Source code">
...
<Demo path="dialog/offset" client:load />
