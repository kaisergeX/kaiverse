...
Components that use this hook: <a href={`${import.meta.env.BASE_URL}/components/slide-action`}>`<SlideAction>`</a>, <a href={`${import.meta.env.BASE_URL}/components/dialog`}>`<Dialog>`</a> mobile variant
